...
// VARY header matching is asymmetric.  If two entries are duplicate depends
// on which entry's response is used in the comparison.  The target response's
// VARY header determines what request headers are examined.  This test verifies
// that Cache.addAll() duplicate checking handles this asymmetric behavior
// correctly.
cache_test(async function(cache, test) {
    ...
    // is a bit confusing, but its necessary to construct a test
    ...
    await fetch(base_url + '?clear-vary-value-override-cookie');
  }, 'Cache.addAll should reject when one entry has a vary header ' +
